...
\begin{frame}
  ...
  \begin{block}{Grundlegende Fragen}
    \begin{itemize}
    \item<3-> Wie Wissen darstellen? \visible<6->{$\quad\Rightarrow\quad$
        \emph{Beschreibungslogiken} }
    \item<4-> Wie Wissen extrahieren? \visible<11->{$\quad\Rightarrow\quad$ \emph{Formale
          Begriffsanalyse}}
    \item<5-> Welches Wissen extrahieren? \visible<12->{$\quad\Rightarrow\quad$
  ...
  \begin{Beispiel}[General Concept Inclusions, GCIs]
    \begin{itemize}
    \item<8-> $\mathsf{Cat} \sqsubseteq \mathsf{Mammal}$
    \item<9-> $\mathsf{Dog} \sqcap \mathsf{Cat} \sqsubseteq \bot$
    \item<10-> $\exists \mathsf{child}. \top \sqsubseteq \mathsf{Parent}$
    \end{itemize}
  \end{Beispiel}
