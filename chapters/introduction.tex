...
the question arises how such knowledge bases can be obtained.  In one way or the other,
...
not only be very time-consuming, but also prone to errors.  An automatic translation
...
for the correctness of extracted GCIs, and if such a GCI is not correct, the expert has to
provide \emph{counterexamples} for it.  This way, the problem that the data may be
\emph{incomplete}, in the sense it lacks crucial counterexamples, can be solved.  This
also has been done in~\cite{Diss-Felix}.
...
description logics when at the beginning of this millennium it was discovered that for the
...
A description logic knowledge base formulated in $\EL$ consists, as most description logic
...
vertices, which we shall call \emph{elements} or \emph{individuals}, are concept names,
and the labels of the edges are role names.  An interpretation is a model of a knowledge
base if all elements \emph{satisfy} the axioms contained in this knowledge base.  For
...
thus provides another link between computer science and mathematics.
...
\end{equation*}
The expert can then either accept this implication, or decline it by providing a
...
\emph{cyclic concept descriptions} using \emph{greatest fixpoint semantics}.
...
attribute exploration, and it can indeed be shown that attribute exploration applied to
...
not only asks the expert after implications which are valid in the current context, but
also after those which only have high confidence.  For this, we first investigate
generalizations of attribute exploration which allow us to explore \emph{sets of
  implications}.  In these generalizations, a set $\mathcal{L}$ of implications can be
specified for which the expert should decide which elements of $\mathcal{L}$ are valid.
In attribute exploration, the set $\mathcal{L}$ is just the set of all valid implications
of a given formal context.  For the case of implications with high confidence, the set
$\mathcal{L}$ is then just the set of all implications whose confidence is above a certain
threshold $c$.
