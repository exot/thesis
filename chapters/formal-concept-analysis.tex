...
\begin{Example}
  ...
  \begin{equation*}
    ...
    \end{array}
  \end{equation*}
  For example, \textsf{BorgQueen} has the attribute \textsf{Artificial}, but not
  \textsf{Honorable}.
\end{Example}
...
$\con K_2$.  Of course, we can drop the parentheses if that does not lead to ambiguity and
...
\begin{Example}
  ...
  other formal concepts do not have $g$ in their extent.  For example, the formal concept
...
\begin{Example}
  ...
  \item If $\phi$ is an order isomorphism from $\alg P = (P, \leq_P)$ to $\alg Q = (Q,
    ...
    Galois connections are a generalization of order isomorphism.
...
\begin{Proof}
  ...
  we can infer $a_1 \leq_P \psi(\phi(a_1))$ by definition~\eqref{eq:4} of a Galois
  connection.
  ...
  is true.  On the other hand, $\psi(\phi(a_1)) \leq_P \psi(\phi(a_1))$, so by the
  definition of a Galois connection, we obtain $\phi(a_1) \leq_Q \phi(\psi(\phi(a_1)))$.
  Since $\leq_Q$ is antisymmetric, equality follows.
\end{Proof}
...
\begin{Proof}
  ...
  Now suppose that~\eqref{eq:9} holds.  Since $\mathcal{L}(U)$ is closed under
...
\begin{Proof}
  ...
  \begin{align*}
    ...
    &= \min\nolimits_{\preceq}\set{ B \subseteq M, \emptyset \precneq B, B = c(B), B \neq
      B''}\\
    &= \mathtt{next-closed-non-intent}(\con K, \leq_M, \emptyset, c)\\
    &= \mathtt{first-closed-non-intent}(\con K, \leq_M, c)
  \end{align*}
  as required.
\end{Proof}
...
how do we compute the implications $A \to B$?  Indeed, it turns out that this can be done
...
we just show an example implementation of attribute exploration in
