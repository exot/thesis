...
\begin{Definition}[Implications, Validity in Formal Contexts]
  ...
  \models \mathcal{J}$, where $\mathcal{J} \subseteq \Imp(M)$ is a set of valid
...
\begin{Definition}[Entailment between Implications]
  ...
  Let $M$ be a set and let $\mathcal{J} \subseteq \Imp(M)$.  Then an implication $(A \to
  B) \in \Imp(M)$ is \emph{entailed by} $\mathcal{J}$, written $\mathcal{J} \models (A \to
  B)$, if and only if for all formal contexts $\con K$ with attribute set $M$ it is true that
  \begin{equation*}
    \con K \models \mathcal{J} \implies \con K \models (A \to B).
  \end{equation*}
  We shall denote with $\Cn_M(\mathcal{J})$ the set of all implications on $M$ which
  follow from $\mathcal{J}$.  We may drop the subscript $M$, and may only write
  $\Cn(\mathcal{J})$, if it is clear from the context.
\end{Definition}
...
whether an implication $A \to B$ follows form $\mathcal{L}$ or not.
...
%  LocalWords:  sep Picard Worf StarFleet
