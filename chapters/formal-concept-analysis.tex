...
\begin{lstlisting}
  ...
  define canonical-base($\con{K} = (G, M, I)$, $\leq_M$, $\mathcal{K} \subseteq \Imp(M)$)
    ;; compute the canonical base of $\con K$ with background knowledge $\mathcal{K}$
    ;; in the lectic order induced by $\leq_M$.
    let ($i$ := 0,
       $P_i$ := first-closed-non-intent($\con{K}$, $\leq_M$, $\mathcal{K}$),
       $\mathcal{L}_i$ := $\emptyset$)
      if $P_i$ = nil then
        return $\mathcal{L}$
      else
        recur ($\mathcal{L}_{i + 1}$ := $\mathcal{L}_i \cup \set{P_i \to P_i''}$,
              $P_{i + 1}$ := next-closed-non-intent($\con{K}$, $\leq_M$, $P_i$, $\mathcal{L}_{i + 1} \cup \mathcal{K}$),
              $i$ := $i + 1$)
    end let
  end define
\end{lstlisting}
...
\todo[inline]{Write: MATH-AL-08-2012, p.\ 13}
