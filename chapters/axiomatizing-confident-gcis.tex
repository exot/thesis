...
\begin{enumerate}[i. ]
\item We want to examine the sets $\Lux(\Idbpedia, 0.95)$ and $\Lux(\Idbpedia,0.90)$ in
  detail, \ie we want to show how large these sets are and which GCIs are contained in
  them.  Moreover, we shall discuss how the ontology engineer proceeds in deciding whether
  the GCIs contained in these sets are true or not.
...
\subsection{Computing Confident Bases of $\Th_c(\Idbpedia)$ for $c = 0.95$ and $c = 0.90$}
\label{sec:comp-conf-bases}

\subsubsection{The case $c = 0.95$}
\label{sec:case-c-=}

\subsubsection{The case $c = 0.90$}
\label{sec:case-c-=-1}
