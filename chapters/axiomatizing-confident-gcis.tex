...
result is quite similar to \Cref{thm:Felix-base-B3}, and we shall do this in
...
An example of such an Infobox is shown in \Cref{fig:infobox-screenshot}.
...
GCIs which can be considered as being too specific are to most common case among all GCIs
...
the constant $d$ depends on the concept description $C$.  To remove this dependency on the
particular concept description we define
\begin{equation}
  \label{eq:35}
  d := \max_{Y \subseteq \Delta^{\mathcal{I}}, Y^{\mathcal{I}} = (A, \mathcal{T})} ( \abs{
    \Delta^{\mathcal{I}} } \cdot \abs{ N_{D}(\mathcal{T}) } + 1 ).
\end{equation}
The set $\mathcal{X}_{\mathcal{I}}$ is then defined as
\begin{equation*}
...
\begin{Lemma}
  ...
  Let $\mathcal{I}$ be a finite interpretation, let let $d \in \NN_0$ as in
  (\Cref{eq:35}).  Then for each $Y \subseteq \Delta^{\mathcal{I}}$
  \begin{enumerate}[i. ]
