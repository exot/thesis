...
to be irredundant anymore.  However, in the case of \Cref{cor:gci-completion-with-S_I},
...
\Cref{thm:Felix-5.18}, which was \cite[Theorem~5.18]{Diss-Felix}.

To show \Cref{thm:minimal-cardinality} we adapt the argumentation of the proof of
\cite[Theorem~5.18]{Diss-Felix} accordingly.  This argumentation in turn is based on a
generalization of \Cref{thm:canonical-base-with-valid-background-knowledge}, stating that
...
\Cref{thm:canonical-base-with-arbitrary-background-knowledge}) we had used the following
...
\begin{Lemma}[Lemma~5.16 of~\cite{Diss-Felix}]
...
To prove \Cref{thm:minimal-cardinality} we also need to introduce another technical
...
\end{equation*}
For the argumentation of the proof of \Cref{thm:minimal-cardinality}, however, we also
...
\begin{Lemma}[Lemmas~5.8 and~5.17 of~\cite{Diss-Felix}]
...
We shall now turn towards the proof of \Cref{thm:minimal-cardinality}, which is a direct
...
The fact that $\bigsqcap \mathcal{L} \cup \mathcal{B}$ is complete for $\mathcal{I}$ has
already been shown in \Cref{thm:gci-completion-simple-version}.  Therefore, it remains to
...
\Cref{thm:canonical-base-with-arbitrary-background-knowledge} which states that
$\mathcal{L} = \Can(\con K_{\mathcal{I}}, \pr_{M_{\mathcal{I}}}(\mathcal{B}) \cup
...
K_{\mathcal{I}}$.  Then by \Cref{thm:canonical-base-with-arbitrary-background-knowledge}
it follows that
\begin{equation*}
  \abs{\mathcal{L}_{\mathcal{C}}} \ge \abs{ \mathcal{L} }.
\end{equation*}
Then since $\abs{ \mathcal{L}_{\mathcal{C}} } \le \abs{ \mathcal{C} }$ and $\abs{
  \bigsqcap \mathcal{L} } = \abs{ \mathcal{L} }$ we obtain that $\abs{ \mathcal{C} } \ge
\abs{ \bigsqcap \mathcal{L} }$ as required.
...
\begin{Proof}[~\Cref{thm:minimal-cardinality}]
  ...
  obtain from \Cref{prop:connection-I-prime-1} that
  \begin{equation*}
    (\pr_{M_{\mathcal{I}}}(\lapprox_{\mathcal{I}}(E)))' = \lapprox_{\mathcal{I}}(E)^{\mathcal{I}}.
  \end{equation*}
  By \Cref{lem:lapprox-is-lower-approximation} we have $\lapprox_{\mathcal{I}}(E)
  ...
  using \Cref{prop:connection-I-prime-1} again yields
  $E^{\mathcal{I}\mathcal{I}\mathcal{I}} =
  ...
  \Cref{lem:characterization-of-completeness} that for all $U \subseteq M_{\mathcal{I}}$,
  ...
  D^{\mathcal{I}}$.  By \Cref{prop:connection-I-prime-2}, $U' = (\bigsqcap
  U)^{\mathcal{I}}$.  From \Cref{lem:mmsc-and-extension-are-galois-connection} we obtain
  ...
  We show that $\mathcal{C} \models (\bigsqcap U \sqsubseteq (\bigsqcap
  U)^{\mathcal{I}\mathcal{I}})$.  For this we observer that $\mathcal{L}_{\mathcal{C}}
  \models (U \to U'')$, since $U$ is closed under $\pr_{M_{\mathcal{I}}}(\mathcal{B}) \cup
  \mathcal{S}_{\mathcal{I}}$, and thus by
  \Cref{lem:implicational-entailment-implies-gci-entailment} we obtain
  \begin{equation*}
    \bigsqcap \mathcal{L}_{\mathcal{C}} \models (\bigsqcap U \sqsubseteq (\bigsqcap U)^{\mathcal{I}\mathcal{I}}).
  \end{equation*}
  On the other hand, if $(\bigsqcap \pr_{M_{\mathcal{I}}}(\lapprox_{\mathcal{I}}(E))
  \sqsubseteq \bigsqcap \pr_{M_{\mathcal{I}}}(E^{\mathcal{I}\mathcal{I}})) \in \bigsqcap
  \mathcal{L}_{\mathcal{C}}$, then $(\lapprox_{\mathcal{I}}(E) \sqsubseteq
  E^{\mathcal{I}\mathcal{I}}) \in \bigsqcap \mathcal{L}_{\mathcal{C}}$ up to equivalence.
  Since $\lapprox_{\mathcal{I}}(E) \sqsubseteq E$ and $(E \sqsubseteq
  E^{\mathcal{I}\mathcal{I}}) \in \mathcal{C}$, we obtain that $\mathcal{C}$ entails all
  GCIs in $\bigsqcap \mathcal{L}_{\mathcal{C}}$.  Thus,
  \begin{equation}
    \label{eq:32}
    \mathcal{C} \models (\bigsqcap U \sqsubseteq (\bigsqcap U)^{\mathcal{I}\mathcal{I}}).
  \end{equation}

  Now, \Cref{lem:Felix-5.16} together with \Cref{eq:31} and \Cref{eq:32} yields the
  ...
  $\mathcal{K}(U) \neq U$, and the proof is finished.

  From \Cref{lem:Felix-5.16} and $\bigsqcap U \sqsubseteq E$ we obtain $\bigsqcap U
  \sqsubseteq \lapprox_{\mathcal{I}}(E)$.  The Galois connection property
  \Cref{lem:pr-bigsqcap-forms-Galois-connection} yields
  ...
  \Cref{lem:characterizing-expressible-in-terms-of}.  But then $\bigsqcap U \sqsubseteq
  E^{\mathcal{I}\mathcal{I}}$, which contradicts \Cref{eq:33}.  Thus
