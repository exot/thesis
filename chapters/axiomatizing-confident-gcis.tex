...
\todo[inline]{Write: say that $\Conf(\mathcal{I}, c) \equiv \bigsqcap \Conf(\con
  K_{\mathcal{I}}, c)$, and same for $\Lux$; this allows to compute these sets by means of
  FCA}
...
We want to compute $\abs{ \Conf(\Idbpedia, c) }$ and $\abs{ \Lux(\Idbpedia, c) }$ for all
$c \in \set{ 0.0, 0.01, 0.02, \dots, 0.99 }$.  To achieve this, we use
\begin{align*}
...
One can see the phenomena of the last two cases also separately: the canonical base of
$\Th(\con K_{\Idbpedia})$ contains 1252 elements, but the canonical base of $\Th(\con
K_{\Idbpedia}) \cup \set{ \emptyset \to \set{ \mathsf{Person} } }$ contains only 1210
implications.  If we also add the implication $\set{ \mathsf{\exists child. \top} } \to
\set{ \mathsf{\exists child. Person} }$, then the size of the canonical base drops to
1163.
