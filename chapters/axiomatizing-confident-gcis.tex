...
\begin{equation*}
  V := \set{ 0.0, 0.01, 0.02, \dots, 0.99 }.
\end{equation*}
To compute $\abs{ \Conf(\Idbpedia, c) }$ and $\abs{ \Lux(\Idbpedia, c) }$ we note that
\begin{align*}
  \abs{ \Conf(\Idbpedia, c) } &= \abs{ \Conf(\con K_\Idbpedia, c) }, \\
  \abs{ \Lux(\Idbpedia, c) } &= \abs{ \Lux(\con K_\Idbpedia, c) }.
\end{align*}
We can therefore conduct the computations in $\con K_\Idbpedia$.  The results are shown in
\Cref{fig:conf-size-behavior}.  Note that the y-axis is scaled logarithmically.
