...
\end{Lemma}
\begin{Proof}
  If $\conf_{\con K}(A \to C) = 0$, then $A' = \emptyset$ and thus the left-hand side of
  \Cref{eq:24} is 0 as well.

  If $A' \neq \emptyset$, then since $A \subseteq B$, it is true that $B' \neq
  \emptyset$.  Therefore we can easily compute
  \begin{align*}
    \conf_{\con K}(A \to C)
...
\begin{Proof}
  ...
  \end{equation*}
  Since $\conf_{\con K}(B_i \to B_{i+1})$ this immediately yields that
  \begin{equation*}
...
\begin{Corollary}
  ...
  Let $\con K = (G, M, I)$ be a formal context, and let $c \in [0,1]$.  If $\mathcal{B}
  \subseteq \Th_c(\con K)$ is complete for $\Lux(\con K, c)$, and if $\mathcal{L}
  \subseteq \Th(\con K)$ is such that $\mathcal{L} \cup \mathcal{B}$ is complete for
  $\Th(\con K)$, then $\mathcal{B}$ is a confident base of $\Th_c(\con K)$ with background
  knowledge $\mathcal{L}$.
\end{Corollary}
