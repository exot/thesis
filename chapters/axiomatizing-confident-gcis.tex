...
To remedy or at least to alleviate this effect of erroneous counterexample we shall
...
objects of those triples were collected into a set $N_C$, \ie they constituted the concept
names of $\Idbpedia$.\footnote{We omitted \texttt{http://www.w3.org/2002/07/owl\#Thing} in
  $N_C$, as it does not introduce any meaningful information} Then, for an $A \in N_C$, we
defined $A^{\Idbpedia}$ to be the set of all subjects that appear in a triple in
$T_{\mathsf{type}}$, \ie
\begin{equation*}
...
We argued intuitively that the number of positive examples should be ``much higher'' then
the number of negative examples.  To formalize this notion, we define the
...
\begin{Proof}
  ...
  \end{equation*}
  such that there are no intents $D$ which $B_i \subsetneq D \subsetneq B_{i+1}$ for all
  $i \in \set{ 0, \dots, n-1 }$.  Then, by induction, \Cref{lem:chain-rule-for-confidence}
  yields
  \begin{equation*}
...
knowledge.  Recall that we defined in \Cref{eq:28} the set
\begin{equation*}
