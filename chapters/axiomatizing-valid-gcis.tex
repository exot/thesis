...
necessary for our further considerations.
...
\begin{Proposition}
  \label{prop:connection-I-prime-4}
  Let $\mathcal{I} = (\Delta^{\mathcal{I}}, \cdot^{\mathcal{I}})$ be a finite
  interpretation, $M$ be a set of concept descriptions and let $B \subseteq M$.  Then
  \begin{equation*}
    B \subseteq \pr_M(\bigsqcap B) \subseteq B'',
  \end{equation*}
  where the derivations are conducted in $\con K_{\mathcal{I}, M}$.
\end{Proposition}
\begin{Proof}
  By \Cref{lem:pr-bigsqcap-forms-Galois-connection}, it is true that $B \subseteq
  \pr_M(\bigsqcap A)$.  Then for a concept description $D$ it is true that
  \begin{align*}
    D \in \pr_M(\bigsqcap B)
    &\iff \bigsqcap B \sqsubseteq D \\
    &\:\implies (\bigsqcap B)^{\mathcal{I}} \subseteq D^{\mathcal{I}} \\
    &\iff B' \subseteq \set{ D }'\\
    &\iff B'' \supseteq \set{ D }'' \ni D \\
    &\iff D \in B'',
  \end{align*}
  using \Cref{prop:connection-I-prime-2} for $(\bigsqcap B)^{\mathcal{I}} = B'$ and the
  definition of $\con K_{\mathcal{I}, M}$ to obtain $D^{\mathcal{I}} = \set{ D }'$.
\end{Proof}
