...
The purpose of this chapter is to prepare this generalization of model exploration to GCIs
...
To this end, we shall first consider in \Cref{sec:class-attr-expl} classical attribute
...
K)$, we shall introduce in \Cref{sec:gener-attr-expl} a generalization of attribute
...
which not.  However, the algorithm we are going to present in \Cref{sec:gener-attr-expl}
...
in the output.  Still, we shall see in \Cref{sec:expl-conf} that this approximative
...
Recall that in attribute exploration as introduced in \Cref{sec:attr-expl} we assumed that
...
Note that we did not require that the set $P$ from \Cref{eq:33} is a $\mathcal{K}$-pseudo
...
At first sight, one may be tempted to say that exploring $\mathcal{L}$ can be achieved (at
...
Intuitively, an algorithm for exploring $\mathcal{L}$ should work as attribute exploration
...
\begin{Definition}[Exploring Sets of Implications]
...
\Cref{alg:attribute-exploration}, and then try to adapt it to our setting of exploring
$\mathcal{L}$.
...
Recall that in \Cref{alg:attribute-exploration} we have used two special functions, namely
...
\Cref{alg:explore-attributes-with-next-closed-none-closed}.  This function suffices to
implement attribute exploration, and the corresponding listing is also shown in
\Cref{alg:explore-attributes-with-next-closed-none-closed}.

We want to use this reformulation as a starting point to discuss an algorithm for
exploring $\mathcal{L}$.  To this end, recall how we have considered attribute exploration
in \Cref{sec:class-attr-expl} as an algorithm for exploring $\Th(\con K)$.  Then in
...
\Cref{alg:explore-attributes-with-next-closed-none-closed} we replace every reference to
...
\end{align*}
The result of this simple replacement is shown in \Cref{alg:explore-implications}.
...
However, \Cref{alg:explore-implications} does not exactly yield an algorithm to explore
...
\Cref{alg:explore-implications} contains too many implications, \ie it may happen that
\begin{equation*}
...
However, we shall show that \Cref{alg:explore-implications} \emph{approximately explores}
...
exploration by confidence, as we shall see in \Cref{sec:poss-fast-expl}.

It is the purpose of the following argumentation to show that
\Cref{alg:explore-implications} approximately explores $\mathcal{L}$ as described above.
...
\begin{Proof}
  ...
  \Cref{prop:technicalities-about-explore-implications} we know that $\mathcal{K}_i
...
We now consider the correctness of \Cref{alg:explore-implications}.
...
\begin{Proof}
  ...
  \Cref{cor:explore-implications-closures-coincide}, we obtain $\mathcal{L}_{n} \subseteq
...
An interesting observation for \Cref{alg:explore-implications} is the following: although
...
\Cref{alg:explore-implications} from
\Cref{alg:explore-attributes-with-next-closed-none-closed} by simple syntactic
substitutions.
...
We now show that \Cref{alg:explore-implications} computes the canonical base of $\Th(p)
...
as given in \Cref{alg:explore-implications} was to be able explore sets of the form
...
thus~\Cref{thm:explore-implications-correctness} yields that
\begin{equation*}
...
discuss in the following a relaxation of \Cref{alg:explore-implications} that allows for
...
\Cref{sec:poss-fast-expl} how this idea can be applied for our case of exploration by
confidence.
...
\Cref{alg:explore-implications-weaker-version} presents a generalization of
\Cref{alg:explore-implications} that uses these ideas.
\Cref{alg:explore-implications-weaker-version} is very similar to
\Cref{alg:explore-implications}, but differs in a crucial aspect: we still compute sets of
the form
\begin{equation*}
...
obtain \Cref{alg:explore-implications} again.

We now argue that \Cref{alg:explore-implications-weaker-version} is indeed an algorithm
...
\begin{Theorem}
  ...
  to \Cref{alg:explore-implications-weaker-version} with this input terminates after
  finitely many steps.
\end{Theorem}

For the correctness of \Cref{alg:explore-implications-weaker-version} we can argue in
practically the same way as we did for \Cref{alg:explore-implications}.  The crucial
...
Now most of the properties of \Cref{alg:explore-implications} carry over to
\Cref{alg:explore-implications-weaker-version}.  However, since we do not necessarily ask
...
\begin{Theorem}
  ...
  \Cref{alg:explore-implications-weaker-version} applied to this input.  Then
  \begin{enumerate}[i. ]
...
\Cref{alg:explore-implications} with $\mathcal{L} = \Th_{c}(\con K)$.  The first problem
we have to face here is how to implement \Cref{lst:explore-implications-1} in the
algorithm, \ie
\begin{equation*}
...
\begin{figure}[tp]
  \centering
  \begin{Algorithm}[Exploration by Confidence, Approximative Version]
...
The algorithm we thus obtain is shown in
\Cref{alg:exploration-by-confidence-first-version}.  The following result is an immediate
...
\begin{Corollary}
  ...
  \cap \Th(p)$.  Then the call of \Cref{alg:exploration-by-confidence-first-version} with
...
\Cref{alg:exploration-by-confidence-first-version} all implications of the form
\begin{equation}
  \label{eq:62}
  P_{i+1} \to \mathcal{L}_{i}(P_{i+1})
\end{equation}
...
\begin{Example}
  ...
  \Cref{fig:simple-exploration-by-confidence-does-not-work}, let $\mathcal{K} = \set{
...
On the other hand, it is possible to ask implications as in~(\ref{eq:63}) instead of those
...
in \Cref{sec:poss-fast-expl} how this can be done.
...
K$, and in particular $(P \to P_{\con K \div \con L_{i}}'') \in \Th_{c}(\con K)$.  The
same argumentation shows that $P_{\con K \div \con L_{i}}'' \subseteq P_{\con L_{i}}''$,
and thus $(P \to P_{\con K \div \con L_{i}}'') \in \Th(\con L_{i})$.  Putting these facts
together we obtain
\begin{equation*}
...
constraint.  Note that for enumerating the sets $A$ that satisfy~\Cref{eq:64}, the Next
...
\Cref{alg:exploration-by-confidence-first-version} has two main flaws: in general, it only
...
\begin{Proof}
  ...
  $X_{\con K \div \con L_{i}}'' \subsetneq A$.  Then $\conf_{\con K}(X_{\con K \div \con
    L_{i}}'' \to \set{m}) = \conf_{\con K}(X \to \set{m}) \ge c$ and \Cref{eq:44} implies
  \begin{equation*}
...
\begin{figure}[tp]
  \begin{Algorithm}[Exploration by Confidence]
...
the context $\con L_{i}$ would contain a counterexample, and thus $m \notin (P_{i})_{\con
  L_{i}}''$.  In this way, the condition in \Cref{eq:44} is ensured.
...
\begin{Proposition}
  ...
  \Cref{alg:exploration-by-confidence-without-Th_c(K)-closures}.  Then for all intents $X
...
To show that \Cref{alg:exploration-by-confidence-without-Th_c(K)-closures} indeed yields
an algorithm that implements exploration by confidence we shall show that it has the form
of \Cref{alg:explore-implications-weaker-version}.  For this we need to show that in every
...
\Cref{alg:exploration-by-confidence-without-Th_c(K)-closures} has the same structure as
...
\begin{Proposition}
  ...
  Suppose that we are in iteration $i$ of
  \Cref{alg:exploration-by-confidence-without-Th_c(K)-closures}.  Let $P \subseteq M$ be
...
It remains to show that $Q_{i+1}$ satisfies \Cref{eq:47}, \ie
\begin{equation*}
...
We have thus shown that \Cref{alg:exploration-by-confidence-without-Th_c(K)-closures} has
the form of \Cref{alg:explore-implications-weaker-version}.  The following result is then
an immediate consequence of \Cref{thm:explore-implications-weaker-version-termination} and
...
\begin{Corollary}
  ...
  \Th_{c}(\con K)$.  Then \Cref{alg:exploration-by-confidence-without-Th_c(K)-closures}
