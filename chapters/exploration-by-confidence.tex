...
section.  Within this description the role of the set $\mathcal{L}$ is played by the set
$\Th(\con K)$.  During the run of the algorithm this set is transferred into sets of the form
\begin{equation*}
  \Th(\con K + C_1 + \dots + C_i)
\end{equation*}
for $C_1, \dots, C_i \subseteq M$.

The operation that transfers $\Th(\con K)$ into $\Th(\con K + C)$ for some $C \subseteq M$
can easily be formulated on the level of implications only.  For this we observe that
$\Th(\con K + C)$ is the set of all implications which are valid in $\con K$ and which are
not invalidated by $C$.  If we denote the empty formal context with attribute set $M$ by
$(\emptyset, M, \emptyset)$, then this amounts to
\begin{equation}
  \label{eq:35}
  \Th(\con K + C) = \Th(\con K) \cap \Th( (\emptyset, M, \emptyset) + C ).
\end{equation}
Using this simple equation we can now replace $\Th(\con K)$ by $\mathcal{L}$ in our
formulation of attribute exploration as given in \Cref{sec:class-attr-expl}.  However, we
can also be more specific and use the original formulation of attribute exploration as
given in \Cref{alg:attribute-exploration} and make the corresponding changes there.

\addfunctionname{explore-implications,next-closed-non-closed}

\begin{figure}[tp]
  \begin{Algorithm}[Reformulating Classical Attribute Exploration]
...
However, the modifications we want to make in the formulation of attribute exploration as
given in \Cref{alg:attribute-exploration} are a bit more comprehensive.  Instead of
utilizing the functions \lstinline{first-closed-non-intent} and
\lstinline{next-closed-non-intent} as discussed in \Cref{alg:canonical-base}, we want to
introduce a function \lstinline{next-closed-non-closed} which returns the lectically
smallest set greater \emph{or equal} to some given set that is closed under a set
$\mathcal{K}$ of implications, but not another set $\mathcal{L}$.  An implementation of
this function is straight forward and given in
\Cref{alg:explore-attributes-with-next-closed-none-closed}.  This listing also includes a
reformulation \lstinline{explore-attributes} that uses \lstinline{next-closed-non-closed}
as well as our newly introduced formalization of a domain expert.
...
define explore-implications($M$, $\leq_M$, $p$, $\mathcal{L} \subseteq \Imp(M)$, $\mathcal{K} \subseteq \Th(p)$)
  ;; Algorithm for exploring sets $\mathcal{L}$ of implications using the expert $p$ and
  ;; the set $\mathcal{K}$ as background knowledge

  $i$ := 0
  $\mathcal{L}_i$ := $\mathcal{L}$
  $\mathcal{K}_i$ := $\mathcal{K}$
  $P_i$ := $\emptyset$
...
We now use this reformulation and include the modifications as discussed above.  The
result is shown in \Cref{alg:explore-implications}.  Note that in
\lstinline{explore-implications} the set $\mathcal{L}_i$ now plays the role of $\Th(\con
K_i)$ in \lstinline{explore-attributes} as shown in
\Cref{alg:explore-attributes-with-next-closed-none-closed}.  In particular, every
occurrence of the closure operator $(\cdot)_{\con K_i}''$ is replaced by
$\mathcal{L}_i(\cdot)$, and \Cref{eq:35} has been used to obtain line
\ref{lst:explore-implications-1} in \Cref{alg:explore-implications}.

We now claim that \lstinline{explore-implications} now yields an algorithm to explore
$\mathcal{L}$, using $p$ as domain expert and $\mathcal{K}$ as background knowledge.  For
this we need to show that \lstinline{explore-implications} always terminates and, upon
termination, yields a base of $\Th(p) \cap \mathcal{L}$ with background knowledge
$\mathcal{K}$.  Moreover, since \lstinline{explore-implications} arose from
\lstinline{explore-attributes} by simple syntactic substitutions it is reasonable to
expect that the base we obtain for $\Th(p) \cap \mathcal{L}$ is actually the canonical
base of this set, \ie \lstinline{explore-implications} computes
\begin{equation*}
  \Can(\Th(p) \cap \mathcal{L}, \mathcal{K}).
\end{equation*}
Showing these claims is the purpose of the rest of this section.
