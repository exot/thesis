...
$M_{i} \subseteq M_{i+1}$ as before, and requiring in addition that
\begin{equation*}
  m_{j} \leq_{M_{i+1}} x
\end{equation*}
...
\begin{lstlisting}
define base/growing-set-of-attributes($\con K = (G, M, I)$, $\leq_{M}$, $\mathcal{S}$)
  $i$ := 0
  $P_i$ := $\emptyset$
  ...
  forever do
    read $\con K_{i+1} = (G, M_{i+1}, I_{i+1})$ where $M_i \subseteq M_{i+1}$ and $I_i = I_{i+1} \cap M_i \times M_i$
    read $\mathcal{S}_{i+1}$ where $\mathcal{S}_i \subseteq \mathcal{S}_{i+1} \subseteq \Th(\con K_{i+1})$

    $\mathcal{L}_{i+1}$ := $\set{ P_r \to (P_r)_{\con K_{i+1}}'' \mid P_r \neq
      (P_r)_{\con K_{i+1}}'', r \in \set{ 0, \ldots, i } }$

    $P_{i+1}$ := next-closure($M_{i+1}$, $\leq_{M_{i+1}}$, $P_{i}$, $\mathcal{L}_{i+1} \cup \mathcal{S}_{i+1})$
    if $P_{i+1} =$ nil exit
